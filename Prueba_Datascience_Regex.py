...
user_name = re.findall('\-\s([\w|-]*)\s', logdata)
time = re.findall('\[([a-zA-Z\d\/\.\s\-\:]*)\]',logdata)
request= re.findall('\\"([a-zA-Z\d\/\.\s\-\:\+\%]*)\\"', logdata)
logdata = [{'host':host, 'user_name': user_name, 'time': time, 'request': request} for host, user_name, time, request in zip(host, user_name, time, request)]
for item in logdata:
    print(item)
input("presiona algo para cerrar")
